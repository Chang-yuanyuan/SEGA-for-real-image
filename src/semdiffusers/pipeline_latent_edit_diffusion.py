...
# from semdiffusers import SemanticEditPipelineOutput
...
class SemanticEditPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        latents = latents * self.scheduler.init_noise_sigma   # 在NoiseCLR中这是1，这里似乎依旧是？
        ...
        for i, t in enumerate(self.progress_bar(timesteps_tensor)):
            ...
            )   # 一张图像的话 enabled_editing_prompts = 1
            ...
            if do_classifier_free_guidance:
                ...
                if enable_edit_guidance:
                    ...
                    )     # len(noise_pred_edit_concepts)是与编辑相关的概念数量。
                    ...
                    for c, noise_pred_edit_concept in enumerate(noise_pred_edit_concepts):
                        # noise_pred_edit_concepts是要编辑的方向
                        ...
                        tmp = torch.quantile(torch.abs(noise_guidance_edit_tmp).flatten(start_dim=2), edit_threshold_c,
                                             dim=2, keepdim=False)
...
def image_grid(imgs, rows, cols, spacing=20):
    ...
    grid = Image.new('RGBA', size=(cols * w + (cols - 1) * spacing, rows * h + (rows - 1) * spacing),
                     color=(255, 255, 255, 0))
...
if __name__ == "__main__":
    ...
    pipe = SemanticEditPipeline.from_pretrained(
        "/media/inspur/disk/yychang_workspace/stable_diffusion_v1_5/stable-diffusion-v1-5/snapshots/module",
        safety_checker=None,
    ).to(device)
    gen = torch.Generator(device=device)
    target = {'editing_prompt': ['sunglasses'], 'reverse_editing_direction': [False],
              'edit_warmup_steps': 10, 'edit_guidance_scale': 6, 'edit_threshold': 0.05, 'edit_momentum_scale': 0.5,
              'edit_mom_beta': 0.6}
